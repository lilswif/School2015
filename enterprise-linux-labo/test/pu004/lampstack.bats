...
@test 'Firewall: interface enp0s8 should be added to the public zone' {
  firewall-cmd --list-all | grep 'interfaces.*enp0s3'
}
...
@test 'Mariadb should have a database for Wordpress' {
  mysql -u root -p${mariadb_root_password} --execute 'use wordpress'
}
...
@test 'MariaDB should not have anonymous users' {
  result=$(mysql -uroot -padmin --execute "select * from user where user='';" mysql)
  [ -z "${result}" ]
}
