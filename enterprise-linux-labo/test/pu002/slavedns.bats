...
assert_forward_lookup() {
  ...
  [ "$ip" = "$(sudo dig @${sut_ip} ${name}.${domain} +short)" ]
}
...
assert_reverse_lookup() {
  ...
  [ "${name}.${domain}." = "$(sudo dig @${sut_ip} -x ${ip} +short)" ]
}
...
assert_alias_lookup() {
  ...
  local name="$2"
  local ip="$3"
  local result="$(sudo dig @${sut_ip} ${alias}.${domain} +short)"
...
@test 'The server should be set up as a slave' {
  result=$(sudo grep 'type slave' /etc/named.conf)
  [ -n "${result}" ]
...
@test 'The server should forward requests to the master server' {
  result=$(sudo grep 'masters' /etc/named.conf | fgrep "${master_dns}" )
  [ -n "${result}" ]
}
